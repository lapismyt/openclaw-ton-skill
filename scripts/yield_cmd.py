...
def _get_pools_fallback(sort_by: str, min_tvl: Optional[float], token: Optional[str], limit: int) -> dict:
    ...
    if result["success"]:
        ...
        if isinstance(pools, list):
            ...
            if token:
                ...
                normalized = [p for p in normalized if 
                    token_upper in (p.get("token0", {}).get("symbol", "").upper()) or
                    token_upper in (p.get("token1", {}).get("symbol", "").upper())
                ]
...
def get_positions(wallet: str, password: Optional[str] = None) -> dict:
    ...
    for j in jettons:
        ...
        if any(kw.lower() in name.lower() or kw.lower() in symbol.lower() for kw in lp_keywords):
            lp_positions.append({
...
"""
    ...
    pools_p.add_argument("--sort", "-s", default="apy", choices=["apy", "tvl", "volume"], help="Sort by")
    ...
    rec_p.add_argument("--risk", "-r", default="medium", choices=["low", "medium", "high"], help="Risk level")
    ...
    dep_p.add_argument("--amount", "-a", type=float, required=True, help="Amount to deposit")
    ...
    with_p.add_argument("--percentage", type=float, default=100, help="Percentage to withdraw (default: 100)")
