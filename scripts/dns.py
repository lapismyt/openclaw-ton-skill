...
def resolve_domain(domain: str) -> dict:
    ...
    return {
        ...
        "wallet_friendly": raw_to_friendly(wallet_address) if wallet_address and ':' in wallet_address else wallet_address,
        "sites": sites,
        "raw_response": data
    }
    ...
    if is_ton_domain(address_or_domain):
        ...
        if result["success"] and result.get("wallet"):
            return {
                ...
                "address_raw": normalize_address(result["wallet"], "raw") if result["wallet"] else None
            }
        else:
            return {
                "success": False,
...
"""
    ...
    # --- check ---
    check_p = subparsers.add_parser("check", help="Check if input is domain and resolve if needed")
