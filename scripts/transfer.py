...
from wallet import WalletStorage, get_account_info, get_jetton_balances
...
def build_jetton_transfer(
    ...
    payload.bits.write_address(Address(wallet.address.to_string()))  # response_destination (excess back to sender)
...
def get_jetton_wallet_address(owner_address: str, jetton_master: str) -> Optional[str]:
    ...
    if jettons["success"]:
        for j in jettons.get("jettons", []):
            if normalize_address(j.get("jetton_address", ""), "raw") == normalize_address(master, "raw"):
...
def transfer_jetton(
    ...
    resolved = resolve_address(to_address)
    if not resolved["success"]:
        return {"success": False, "error": f"Cannot resolve recipient: {resolved.get('error')}"}
    recipient = resolved["address"]
    ...
    for j in jettons.get("jettons", []):
        ...
        if j.get("jetton_address") == jetton:
            ...
            break
        try:
            if normalize_address(j.get("jetton_address", ""), "raw") == normalize_address(jetton, "raw"):
                target_jetton = j
                break
        except:
            pass
    ...
    boc = build_jetton_transfer(
        wallet=wallet,
        jetton_wallet_address=raw_to_friendly(jetton_wallet_address) if ':' in jetton_wallet_address else jetton_wallet_address,
        to_address=recipient,
        amount=amount_raw,
...
"""
    ...
    ton_p.add_argument("--from", "-f", dest="from_wallet", required=True, help="Sender wallet (label or address)")
    ton_p.add_argument("--to", "-t", required=True, help="Recipient (address or .ton domain)")
    ton_p.add_argument("--amount", "-a", type=float, required=True, help="Amount in TON")
    ton_p.add_argument("--comment", "-c", help="Transfer comment")
    ton_p.add_argument("--confirm", action="store_true", help="Confirm and send (otherwise just emulate)")
    
    # --- jetton ---
    jetton_p = subparsers.add_parser("jetton", help="Transfer Jetton")
    jetton_p.add_argument("--from", "-f", dest="from_wallet", required=True, help="Sender wallet")
    jetton_p.add_argument("--to", "-t", required=True, help="Recipient (address or .ton domain)")
    jetton_p.add_argument("--jetton", "-j", required=True, help="Jetton symbol or address")
