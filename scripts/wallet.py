...
from utils import (
    ...
    load_config, save_config, ensure_skill_dir,
    tonapi_request, normalize_address, is_valid_address,
    raw_to_friendly, friendly_to_raw,
    SKILL_DIR, WALLETS_FILE
)
...
class WalletStorage:
    ...
    def get_wallets(self, include_secrets: bool = False) -> List[dict]:
        ...
        if not include_secrets:
            ...
            return [
                {k: v for k, v in w.items() if k not in ("mnemonic", "private_key", "secret_key")}
                for w in wallets
            ]
        return wallets
    
    def get_wallet(self, identifier: str, include_secrets: bool = False) -> Optional[dict]:
        """
    ...
    def update_wallet(self, identifier: str, updates: dict) -> bool:
        ...
        for i, w in enumerate(storage["wallets"]):
            if w.get("label", "").lower() == identifier.lower() or w.get("address") == identifier:
    ...
    if result["success"]:
        ...
        for item in balances:
            ...
            jettons.append({
                ...
                "balance_human": float(item.get("balance", 0)) / (10 ** jetton.get("decimals", 9)),
...
def cmd_import(args, password: str) -> dict:
    ...
    if len(mnemonic) != 24:
        return {"success": False, "error": f"Мнемоника должна быть 24 слова, получено {len(mnemonic)}"}
...
"""
    ...
    parser.add_argument("--password", "-p", help="Encryption password (or use WALLET_PASSWORD env)")
    ...
    create_p.add_argument("--version", "-v", default="v4r2", choices=["v3r2", "v4r2"], help="Wallet version")
    
    # --- import ---
    import_p = subparsers.add_parser("import", help="Import wallet from mnemonic")
    import_p.add_argument("--mnemonic", "-m", required=True, help="24-word mnemonic (space-separated)")
    import_p.add_argument("--label", "-l", help="Wallet label")
    import_p.add_argument("--version", "-v", default="v4r2", choices=["v3r2", "v4r2"], help="Wallet version")
    
    # --- list ---
    list_p = subparsers.add_parser("list", help="List all wallets")
    list_p.add_argument("--balances", "-b", action="store_true", help="Include balances (slower)")
