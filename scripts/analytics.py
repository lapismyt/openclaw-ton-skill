...
def analyze_price_history(token: str, days: int = 7) -> dict:
    ...
    if not prices:
        return {
            "success": False,
            "error": "No valid prices in history"
        }
    ...
    return {
        ...
        "volatility": {
            ...
            "level": "high" if volatility_pct > 20 else ("medium" if volatility_pct > 10 else "low")
        },
...
def analyze_pools(token: str) -> dict:
    ...
    for pool in pools:
        ...
        dex_stats[dex]["pairs"].append({
            "pair": pool.get("pair_name") or f"{pool.get('token0_symbol')}/{pool.get('token1_symbol')}",
            "liquidity": liq,
            "volume_24h": vol
        })
    ...
    top_pools = sorted(pools, key=lambda x: x.get("liquidity", 0) or 0, reverse=True)[:5]
...
def compare_tokens_detailed(tokens: List[str]) -> dict:
    ...
    if len(results) > 1:
        ...
        # По trust
        by_trust = sorted(results, key=lambda x: x.get("trust_score") or 0, reverse=True)
    ...
    return {
        ...
        "best_by_trust": max(results, key=lambda x: x.get("trust_score") or 0)["symbol"] if results else None
    }
    ...
    try:
        if args.command == "info":
            ...
            result = {
                ...
                "known_tokens": [
                    {"symbol": k, "address": v}
                    for k, v in sorted(KNOWN_TOKENS.items())
                ],
