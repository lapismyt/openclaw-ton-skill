...
def parse_transaction(tx: dict, wallet_label: str, wallet_address: str) -> Optional[dict]:
    """
    ...
    try:
        ...
        if action_type == "TonTransfer":
            ...
            event["type"] = "swap"
            event["dex"] = dex
            event["amount"] = f"{human_in:.4f} {symbol_in} → {human_out:.4f} {symbol_out}"
            ...
            event["description"] = action.get("simple_preview", {}).get("description", action_type)
...
def emit_event(event: dict, logger: logging.Logger) -> None:
    ...
    # Лог
    logger.info(f"Event: {event.get('type')} | {event.get('wallet')} | {event.get('amount', 'N/A')}")
...
class SSEMonitor:
    ...
    def start(self) -> None:
        ...
        if not SSE_AVAILABLE:
            raise RuntimeError("sseclient not available. Install: pip install sseclient-py")
        ...
        for event in client.events():
            ...
            if event.event == "message" and event.data:
                try:
                    ...
                    self.logger.warning(f"Invalid JSON in SSE event: {event.data[:100]}")
...
class PollingMonitor:
    ...
    def start(self) -> None:
        ...
        self.logger.info(f"Starting polling monitor: {len(self.addresses)} addresses, {self.interval}s interval")
    ...
    def _check_address(self, address: str) -> None:
        ...
        if not result["success"]:
            self.logger.warning(f"Failed to get events for {label}: {result.get('error')}")
            return
    ...
    def start(self) -> None:
        ...
        if self.wallet_filter:
            ...
            for w in all_wallets:
                if w.get("label") in self.wallet_filter or w.get("address") in self.wallet_filter:
                    wallets.append(w)
        else:
            wallets = all_wallets
        ...
        if not wallets:
            ...
            print(json.dumps({"error": f"No wallets matching filter: {self.wallet_filter}"}))
            sys.exit(1)
...
def cmd_start(args) -> None:
    ...
    if not password:
        if sys.stdin.isatty():
            ...
            print(json.dumps({"error": "Password required. Use -p or WALLET_PASSWORD env"}))
            sys.exit(1)
...
"""
    ...
    start_p.add_argument("--wallet", "-w", action="append", help="Specific wallet(s) to monitor")
    start_p.add_argument("--daemon", "-d", action="store_true", help="Run as daemon")
    start_p.add_argument("--polling", action="store_true", help="Use polling instead of SSE")
    start_p.add_argument("--verbose", "-v", action="store_true", help="Verbose logging")
    
    # --- status ---
    status_p = subparsers.add_parser("status", help="Show monitor status")
    
    # --- stop ---
    stop_p = subparsers.add_parser("stop", help="Stop monitoring")
